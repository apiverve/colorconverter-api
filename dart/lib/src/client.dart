...
class ColorconverterValidationException implements Exception {
  final List<String> errors;

  ColorconverterValidationException(this.errors);

  @override
  String toString() => 'ColorconverterValidationException: ${errors.join("; ")}';
}
...
/// Color Converter API client.
///
/// For more information, visit: https://apiverve.com/marketplace/colorconverter?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [hex] (required) - The color in hexadecimal format (e.g., FF0000). Make sure you ommit the #
class ColorconverterClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'hex': ValidationRule(type: 'string', required: true),
  };

  ColorconverterClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/colorconverter',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ColorconverterValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ColorconverterValidationException(errors);
    }
  }

  /// Execute a request to the Color Converter API.
  ///
  /// Parameters are validated before sending the request.
  Future<ColorconverterResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ColorconverterException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ColorconverterResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ColorconverterException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ColorconverterException('Resource not found');
      } else {
        throw ColorconverterException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ColorconverterException) rethrow;
      throw ColorconverterException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Color Converter API client.
class ColorconverterException implements Exception {
  final String message;

  ColorconverterException(this.message);

  @override
  String toString() => 'ColorconverterException: $message';
}
